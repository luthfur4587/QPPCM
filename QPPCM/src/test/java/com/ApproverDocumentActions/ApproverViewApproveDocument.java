...
public class ApproverViewApproveDocument extends ApproverDocumentBases{
	...
	public void TestName(String username, String wrongPwd, String Title, String Description, String Year, String Category, String ResourceType, 
		...
		try {
			...
			wait(2);
			System.out.println(getElementByPassingXPath("//app-approve-reject/section/div/div/div/p").getText());
			if (getElementByPassingXPath("//app-approve-reject/section/div/div/div/p").getText().equals(docTitle)==false){
				reportWarning("Document Title mismatch on the Edit Document page");
			}
			
			
			click("ApproverApproveButton_xpath");
			wait(2);
			click("PublishCheckBox_xpath");
			wait(2);
			click("PopupApproveSubmitButton_xpath");
			wait(2);
			if (getElement("SuccessBanner_xpath").getText().equals("Success! The content has been approved/published in the Resource Library page and is listed below.")){
				reportPass("Confirmation Success");
				reportPass("Following Document is published: "+ docTitle);
			}else{reportWarning("Published Confirmation failed");}

			//Approver Logout
			doLoout();
			
			//Author login back to check duplicate Approved content 
			doLogin(Param.getProperty("AuthorUserID"), Param.getProperty("AuthorPassword"));
			...
			for (int i=0; i<PendingTitleCounts; i++){
				String PendingTitleText=Pendingtitles.get(i).getText();
				if (PendingTitleText.equals(docTitle)){
					duplicateCount=duplicateCount+1;
					if (duplicateCount>=2){
						reportWarning("Duplicate Content in the Pending & Approved Content table");
					}
				}
			}

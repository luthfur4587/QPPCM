...
public class WebinarMigration extends ApplicationDataMigrationBase{
...
//		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
	...
	@Test(dataProvider="createWebinar")
...
//			driver.get("/html/body/app-root/header/div/div/div[1]/a/img");
			getElement("AddWebinarButton_xpath").click();
			if (getElement("PageTitle_xpath").getText().equals("Add Webinar Materials")==false){
				reportWarning("Page Title mismatch");
			}
			
			String webinarTitle=Title+"-"+getRandomInt(1000);
//			String webinarTitle=Title.trim();
			type("Title_xpath", webinarTitle);
...
//			click("CancelButton_xpath");
			...
			if (getElement("SuccessBanner_xpath").getText().equals("Success! Your content has been submitted for approval & is listed below.")){
				...
				reportPass("Following Document has been submitted for approval: "+ webinarTitle.toString());
			}else{reportWarning("Confirmation failed");}
			System.out.println(webinarTitle);
			System.out.println(webinarTitle.toString());
			
			wait(1);
			//Verify the Pending Approval status 
			getElementByPassingXPath("//a[contains(text(),'"+webinarTitle.trim()+"')]").click();
			wait(1);

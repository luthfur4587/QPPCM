...
public class DocumentMigration extends ApplicationDataMigrationBase{
...
//			click("CancelButton_xpath");
			...
			if (getElement("SuccessBanner_xpath").getText().equals("Success! Your content has been submitted for approval & is listed below.")){
				...
				reportPass("Following Document has been submitted for approval: "+ docTitle.toString());
			}else{reportWarning("Confirmation failed");}
			System.out.println(docTitle);
			System.out.println(docTitle.toString());
			
			wait(1);
			//Verify the Pending Approval status 
			getElementByPassingXPath("//a[contains(text(),'"+docTitle.trim()+"')]").click();
			wait(1);
